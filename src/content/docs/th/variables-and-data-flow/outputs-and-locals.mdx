...
**`local`** คือ expression ที่มีชื่อ คำนวณครั้งเดียวภายใน configuration แล้วใช้ซ้ำได้ทุกที่ที่ต้องการผลลัพธ์ — ไม่มีใครนอก configuration ตั้งค่าหรืออ่านได้โดยตรง ทำให้ต่างจากทั้ง **`variable`** ที่ caller ตั้งค่าให้ และ **`output`** ที่มองเห็นได้จากภายนอก **`output`** block เปิดเผยค่าหลัง `apply` อ่านได้ด้วย `terraform output`, `terraform output <name>` หรืออ่านแบบ machine-readable ผ่าน `terraform output -json` `sensitive = true` ซ่อนค่าจาก CLI output ปกติเหมือนกับ variable โดยไม่ได้ลบออกจาก **state** นอกเหนือจาก root configuration เดียว output คือสิ่งที่ทำให้ composition เป็นไปได้ — นั่นคือวิธีที่ module ส่งค่ากลับไปให้ caller และวิธีที่ `dependency` block ใน Terragrunt อ่านค่าจาก unit หนึ่งที่ deploy แยกกันไปยังอีก unit หนึ่ง
</Callout>
...
export const quizOutputsAndLocalsAzure = [
  {
    ...
    options: [
      ...
      'ไม่มีความต่างจริง ๆ เป็น syntax ที่ใช้แทนกันได้สำหรับ feature เดียวกัน',
