...
**`count`** สร้าง resource N copy อ้างอิงด้วย index ตัวเลข (`resource[0]`, `resource[1]`) และจุดอ่อนหลักคือการลบ item ออกจากตรงกลางของ list ที่อยู่เบื้องหลังจะทำให้ index ทุกตัวหลังจากนั้นเลื่อนลงมาหนึ่งตำแหน่ง เปลี่ยน "ลบแค่หนึ่งอย่าง" ให้กลายเป็น cascade ของการ destroy และ recreate ทุกอย่างที่ตามมา **`for_each`** สร้าง instance หนึ่งตัวต่อหนึ่ง key ใน map หรือ set ของ string อ้างอิงด้วย key ที่เสถียรนั้น (`resource["primary"]`) แทนที่จะเป็นตำแหน่ง ทำให้ลบ key เดียวก็ destroy แค่ instance นั้น instance ของ key อื่นทุกตัวไม่ถูกกระทบเลย **`for` expression** (`[for x in list : expr]` หรือ `{for k, v in map : k => expr}`) คือ comprehension แบบทั่วไปที่ไม่เกี่ยวข้องกัน ใช้แปลง collection หนึ่งให้เป็นอีก collection หนึ่ง มีชื่อคล้าย `for_each` แต่จุดประสงค์คนละเรื่อง และไม่เคยตัดสินว่า resource instance มีกี่ตัว
</Callout>
...
export const quizCountForEachAzure = [
  {
    ...
    options: [
      ...
      'Terraform จะปฏิเสธไม่ plan จนกว่าจะคืน list ให้ยาวเท่าเดิม',
    ],
    answer: 1,
  },
  {
  ...
  },
  {
    ...
    options: [
      ...
      'for_each กำลังจะเลิกใช้แล้วเปลี่ยนไปใช้ for expression แทน',
    ],
    answer: 1,
  },
  {
    q: 'instance ที่สร้างด้วย for_each ถูกอ้างอิงจากภายนอก resource block อย่างไร',
    options: [
      'ด้วย index ตัวเลขอย่าง resource[0]',
      'ด้วย key ที่เสถียรของตัวเอง อย่าง resource["primary"]',
      'ด้วยลำดับที่ปรากฏในไฟล์ configuration',
