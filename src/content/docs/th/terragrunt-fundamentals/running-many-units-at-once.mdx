...
`terragrunt run --all plan` และ `terragrunt run --all apply` คือ command ปัจจุบันสำหรับรันทุก unit ที่อยู่ใต้ directory หนึ่ง มาแทน syntax เก่า `terragrunt run-all <command>` ที่ถูก deprecated ไปแล้ว ในขณะที่ `terragrunt plan-all` และ `terragrunt apply-all` แบบเก่ากว่านั้นถูกลบทิ้งไปเลย และจะ fail ถ้าลองรันดู **--filter** เช่นใน `terragrunt run --all --filter './subscription-a/**' -- plan` จำกัดขอบเขตการรันให้เหลือแค่บาง unit มาแทน flag เก่าอย่าง `--queue-include-dir`/`--queue-exclude-dir` ส่วน **--non-interactive** มาแทน `--terragrunt-non-interactive` สำหรับการรันแบบไม่มีคนคอย confirm ใน CI **before_hook** และ **after_hook** block ใน `terraform` block ของ unit รัน shell command ใด ๆ ก็ได้ ก่อนหรือหลัง operation หนึ่งบน unit นั้น เช่น validation script หรือการแจ้งเตือน Slack ส่วนลำดับที่ unit ต่าง ๆ ถูกประมวลผลจริงระหว่าง `run --all apply` ไม่เคยเป็นสิ่งที่ต้องระบุด้วยมือเลย มันมาจาก dependency graph ที่สร้างจากทุก `dependency` block ทั่วทั้ง tree ตามที่บทก่อนหน้ากล่าวไว้
</Callout>
