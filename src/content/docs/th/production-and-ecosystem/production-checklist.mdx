...
export const quizProductionChecklist = [
  {
    ...
    options: [
      ...
      'local state แปลว่าเครื่องเดียวถือสำเนาความจริงหนึ่งเดียวของ Azure footprint และ apply พร้อมกันจาก engineer หลายคนทำให้ state corrupt ได้ถ้าไม่มี locking',
  ...
  },
  {
    ...
    options: [
      ...
      'เพราะ -/+ ใน plan แปลว่า resource ของ Azure ที่มี state จะโดนทำลายแล้วสร้างใหม่ ซึ่งอาจแปลว่ามี data loss หรือ downtime จริงทันทีที่มีคน approve',
  ...
  },
  {
    ...
    options: [
      'ตัดความจำเป็นของ state file แยกต่อ unit ออกไป',
      'ทำให้ unit หนึ่งใช้ output จริงของอีก unit ได้อัตโนมัติ โดย Terragrunt คำนวณลำดับ apply ที่ถูกต้องเอง แทนที่จะพึ่งให้คนจำลำดับที่ถูกต้องด้วยมือ',
      'แทนที่ความจำเป็นของ remote backend ไปเลยทั้งหมด',
      'ทำให้ unit หลายตัวใช้ terragrunt.hcl ไฟล์เดียวร่วมกันได้',
    ],
    answer: 1,
  },
  {
