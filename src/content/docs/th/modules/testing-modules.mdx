...
## confidence สามระดับ และทำไมแทนกันไม่ได้
...
`terraform validate`, `terraform plan` และ `terraform test` คือ check สามแบบที่ต่างกันจริง ๆ ไม่ใช่ของแทนกันได้: `validate` เช็คแค่ HCL syntax กับความสอดคล้องภายใน และไม่คุยกับ Azure เลย `plan` คุยกับ Azure API จริงและโชว์ provider-side error แต่ไม่ assert ว่าผลลัพธ์ถูกต้องไหม และ `terraform test` คือตัวเดียวที่ assert behavior เฉพาะที่คาดไว้จริง ๆ ผ่าน `.tftest.hcl` file, `run` block และ `assert` block ที่มี `condition` กับ `error_message` ภายใน `run` block, `command = plan` เร็ว ไม่สร้าง resource จริง และครอบคลุม test case ส่วนใหญ่ ส่วน `command = apply` สร้างและ destroy resource จริง เพื่อให้ assert กับ value ที่ Azure คำนวณให้หลัง apply จริงเท่านั้น — integration testing จริงแลกมากับความเร็วและ cloud resource ที่มีอยู่จริงในช่วงสั้น ๆ
</Callout>
...
export const quizTestingModules = [
  {
    ...
    options: [
      'เช็คได้ทุกอย่าง รวมถึง VM size ที่เลือกมีอยู่จริงใน Azure region นั้นไหม',
      'เช็คแค่ HCL syntax กับความสอดคล้องภายในของ configuration และไม่คุยกับ Azure เลย จึงจับอย่าง VM size ที่ไม่มีจริงหรือชื่อ storage account ที่ถูกใช้ไปแล้วไม่ได้',
      'คุยกับ Azure API จริงเหมือน plan จึงจับ error แบบเดียวกับที่ plan จับได้',
      'เช็คแค่ว่า resource name ไม่ซ้ำกันทั้ง subscription เท่านั้น',
    ],
    answer: 1,
  },
  {
  ...
  },
  {
    ...
    options: [
      ...
      'command = plan เร็วและไม่สร้าง resource จริง เช็คแค่ planned value ส่วน command = apply สร้าง resource จริง assert กับ attribute จริงที่ได้ แล้ว teardown ทีหลัง',
