...
`source` is the provider's registry address (`hashicorp/azurerm` resolves to the official Azure provider on the Terraform Registry), and `version` is a constraint, not a pin — `~> 5.0` allows any `5.x` release.
