...
description: 'Installing Terraform with a version manager like tenv, a minimal real Azure project, the conventional file layout, and the terraform fmt and validate commands.'
sidebar:
  order: 5
---
...
Install Terraform through a **version manager** like `tenv` (which also handles OpenTofu and Terragrunt) instead of a single pinned binary; a minimal real project is just a `provider` block plus an `azurerm_resource_group` plus one resource like `azurerm_storage_account` inside it — remembering that a storage account name must be globally unique across all of Azure; the `main.tf` / `variables.tf` / `outputs.tf` / `providers.tf` / `versions.tf` split is a **human convention** — Terraform loads every `.tf` file in a directory as one combined configuration regardless of filename; and `terraform fmt` (canonical formatting) plus `terraform validate` (internal syntax and consistency checks) are cheap, worthwhile Day-1 habits — but `validate` never talks to the real Azure API, so it cannot catch a quota limit, a taken storage account name, or a missing permission, which is what `plan` is for.
</Callout>
...
export const quizInstall = [
  ...
  },
  {
    ...
    options: [
      'It checks the configuration against the real Azure API',
  ...
  },
  {
    ...
    options: [
      'Whether the Azure subscription has hit a service quota',
      'Internal syntax and consistency of the configuration, such as valid HCL and correctly typed arguments, without contacting Azure',
      'Whether the storage account name you chose is already taken globally',
      'Whether your credentials have permission to create the resources',
    ],
    answer: 1,
  },
  {
    q: 'Why is passing terraform validate not enough to guarantee a real Azure-side problem will not occur',
    options: [
      'validate is only a linter for HCL keywords and skips resource blocks entirely',
      'validate never contacts the real Azure API, so it cannot catch things like quota limits, a globally taken storage account name, or missing permissions — only plan or apply can surface those',
      'validate only works for the azurerm_storage_account resource type',
